...
public class GameMenu {
	...
	private void appleFarm() {
		...
		while(b) {
			...
			if(hp <= 0) {
				...
				while(flag) {
					...
					if(list.size() == 0) {
						...
						System.out.println("현재 갖고 계신 아이템");
						for (int i = 0; i < list.size(); i++) {
							System.out.print(list.get(i).getItemName() + list.get(i).getCount()+ "개  ");
						}
						System.out.println();
			...
			if(selectHp.getHp() > 0) {
				farmInfo();
				System.out.println("\n");
				System.out.println("1.나무심기 2.나무정보 3.물주기 4.영양제주기");
				System.out.println("5.가지치기 6.수확하기 7.나가기");
				int menu = sc.nextInt();
					if(menu == 1) {
						insertTree();
					} else if(menu == 2) {
	...
	private void getApple() {
		...
		vo = as.selectApple(index);
		System.out.println(vo.toString());
		System.out.println(vo.getNum() == index && vo.getWater() == 5 && vo.getNutrients() == 1 && vo.getPruning() == 2);
		System.out.println(vo.getNum());
		System.out.println(vo.getWater());
		System.out.println(vo.getNutrients());
		System.out.println(vo.getPruning());
		if(vo.getNum() == index && vo.getWater() == 5 && vo.getNutrients() == 1 && vo.getPruning() == 2) {
						...
						if(charVO.getMoney() < 1000) {
							...
							for (int i = 0; i < list.size(); i++) {
								if(list.get(i).getItemName().equals("오렌지주스")) {
									...
									gs.updateDownMoney(1000);
									System.out.println("오렌지주스 구입 완료!");
									MainMenu.clearScreen();
								}
							}
							if(c) {
						...
						if(charVO.getMoney() < 3000) {
							...
							for (int i = 0; i < list.size(); i++) {
								if(list.get(i).getItemName().equals("네잎클로버")) {
									...
									System.out.println("네잎클로버 구입 완료!");
